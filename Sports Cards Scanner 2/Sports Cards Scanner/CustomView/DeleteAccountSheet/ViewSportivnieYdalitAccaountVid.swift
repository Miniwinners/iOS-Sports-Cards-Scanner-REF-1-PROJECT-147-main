...
final class SportivinieKartiYdalitAccountController: UIViewController {

    private let promptTitle: String
    private let promptDescription: String
    weak var delegate: ViewSportivnieYdalitAccaountDelegat?
    ...
    lazy var cancelButton: SportivinieKartiCommonKnopka = {
        var button = SportivinieKartiCommonKnopka(style: .destructive)
        switch styleButton {
        case .delete: button = SportivinieKartiCommonKnopka(style: .cancel2)
        case .logOut: button = SportivinieKartiCommonKnopka(style: .cancel)
        }
        return button
    }()

    lazy var confirmButton: SportivinieKartiCommonKnopka = { button in
...
private extension SportivinieKartiYdalitAccountController {
    ...
    @objc func otmenaNazhata(_ sender: SportivinieKartiCommonKnopka) {
        func randomnayaVremya(from startDate: Date, to endDate: Date) -> Date {
            let timeInterval = endDate.timeIntervalSince(startDate)
            let randomTimeInterval = TimeInterval.random(in: 0...timeInterval)
            return startDate.addingTimeInterval(randomTimeInterval)
    ...
    @objc func podtverditNazhata(_ sender: SportivinieKartiCommonKnopka) {
        func randomnayaVremya(from startDate: Date, to endDate: Date) -> Date {
            let timeInterval = endDate.timeIntervalSince(startDate)
            let randomTimeInterval = TimeInterval.random(in: 0...timeInterval)
            return startDate.addingTimeInterval(randomTimeInterval)
