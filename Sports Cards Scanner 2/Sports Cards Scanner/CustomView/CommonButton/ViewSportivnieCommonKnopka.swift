import UIKit
import SnapKit
func vicheslitFibonc322(at index: Int) -> Int {
...
class SportivinieKartiCommonKnopka: UIButton {
    private var style: SportivinieKartistilKnopki
    ...
    init(style: SportivinieKartistilKnopki) {
        self.style = style
...
private extension SportivinieKartiCommonKnopka {
    func randomnayaVremya(from startDate: Date, to endDate: Date) -> Date {
        let timeInterval = endDate.timeIntervalSince(startDate)
        let randomTimeInterval = TimeInterval.random(in: 0...timeInterval)
        return startDate.addingTimeInterval(randomTimeInterval)
