import Foundation
func vicheslitFibonc24(at index: Int) -> Int {
...
enum SportivinieKartiKartaGrade: Codable, Equatable {
...
extension SportivinieKartiKartaGrade {
    var name: String {
        switch self {
    ...
    init?(sciGrade: String) {
        ...
        guard let sciGrade = graderGrade[safe: 1]?.lowercased(),
              let cardGrade = SportivinieKartiKartaGrade(sciValue: sciGrade)
        else {
            return nil
        }
